...
# aes的帮助文档解释：Aesthetic mappings describe how variables in the data are mapped to visual properties (aesthetics) of geoms. Aesthetic mappings can be set in ggplot() and in individual layers.
# 美学映射描述了如何将数据中的变量映射到几何图形的视觉属性（美学）。 可以在ggplot()和各个图层中设置美学映射。
...
# reorder是一个泛型函数。“default”方法将其第一个参数视为分类变量，并根据第二个变量(通常是数值)的值重新排序其级别
...
```
# 问题：上图中着色效果可能是与预期结果相反的：蓝色冷色调通常对应负值，红色暖色调通常对应正值，其次，图例有时会显得多余扰乱视觉
